...
def generate_visibility_sorted_catalog(all_manifests: Dict) -> Dict:
    ...
    Note: This catalog includes ALL vendors regardless of displayOnWebsite setting.
    The displayOnWebsite flag should be used by consumers to filter the display.
    """
    visibility_catalog = {
        'high': [],
        'medium': [],
    ...
    for vendor_id, manifest in all_manifests.items():
        visibility = manifest.get('visibility', 'low')

        # Ensure visibility is valid, default to 'low' if not
    ...
    for tier in visibility_catalog:
        visibility_catalog[tier].sort(key=lambda x: x['name'].lower())
    ...
    return {
        'metadata': {
            ...
            'highCount': len(visibility_catalog['high']),
            'mediumCount': len(visibility_catalog['medium']),
            'lowCount': len(visibility_catalog['low'])
        },
        'high': visibility_catalog['high'],
        'medium': visibility_catalog['medium'],
        'low': visibility_catalog['low']
    }
...
def main():
    ...
    # Generate visibility-sorted catalog
    print("\nGenerating visibility-sorted catalog...")
    visibility_catalog = generate_visibility_sorted_catalog(all_manifests)

    visibility_json_path = catalogs_path / 'integration-catalog-visibility-sorted.json'
    with open(visibility_json_path, 'w', encoding='utf-8') as f:
        json.dump(visibility_catalog, f, indent=2, ensure_ascii=False)
    print(f"✓ Visibility-sorted catalog (JSON): {visibility_json_path}")

    visibility_yaml_path = catalogs_path / 'integration-catalog-visibility-sorted.yml'
    with open(visibility_yaml_path, 'w', encoding='utf-8') as f:
        f.write("# AUTO-GENERATED - DO NOT EDIT\n")
        f.write("# This file is generated by generate-catalogs.py\n")
        f.write("# Vendors sorted by visibility tier (high, medium, low), then alphabetically\n\n")
        yaml.dump(visibility_catalog, f, default_flow_style=False, allow_unicode=True, sort_keys=False, width=120)
    print(f"✓ Visibility-sorted catalog (YAML): {visibility_yaml_path}")
