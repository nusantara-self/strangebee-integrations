...
def main():
    ...
    if catalogs_path.exists():
        for vendor_dir in catalogs_path.iterdir():
            if vendor_dir.is_dir() and vendor_dir.name not in vendors and vendor_dir.name not in ['functions-catalog.json', 'external-integrations']:
    ...
    print(f"\n✓ Generated {len(vendors)} vendor catalogs")
    ...
    functions_catalog_json_path = catalogs_path / 'functions-catalog.json'
    ...
    functions_catalog_yaml_path = catalogs_path / 'functions-catalog.yml'
